...
switch lower(options.('RBFFunction'))
      ...
      case 'invquadratic'
        options.('rbfphi')   = @rbfphi_inverse_quadric;
      case 'invmultiquadric'
        options.('rbfphi')   = @rbfphi_inverse_multiquadrics;
    otherwise
        options.('rbfphi')   = @rbfphi_linear;
end
...
%inverse
rbfcoeff=A\b; %is computed differently than x = inv(A)*b and is recommended for solving systems of linear equations
...
function u=rbfphi_inverse_quadric(r, const)
u=1./(1+r.*r/(const*const));

function u=rbfphi_inverse_multiquadrics(r, const)
u=1./sqrt(1+r.*r/(const*const));